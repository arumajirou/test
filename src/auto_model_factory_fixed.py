...
class AutoModelFactory:
    ...
    def _prepare_config(
        ...
        if self.config.backend == "optuna":
            ...
            def config_fn(trial):
                ...
                # NOTE: Optuna warns if choices are non-primitive types. NF expects nested shapes.
                if self.model_name in ["NHITS", "NBEATS"]:
                    # n_blocks: a 3-tuple works for NF; warning from Optuna is harmless.
                    cfg["n_blocks"] = trial.suggest_categorical("n_blocks", [(1, 1, 1), (2, 2, 2), (3, 3, 3)])
                    # mlp_units: nested shape like [[512,512]]; we use tuple-of-tuple to keep structure.
    ...
    def _create_and_optimize(
        ...
        **kwargs,
        ...
        """Create Auto* model and run optimization safely."""
        # Imports
        try:
        ...
        # Convert pandas DataFrame -> TimeSeriesDataset if needed
        dataset_obj = dataset
        try:
            if isinstance(dataset, pd.DataFrame):
                dataset_obj, *_ = TimeSeriesDataset.from_df(dataset)
        except Exception as e:
            logger.warning(f"Falling back to raw DataFrame for dataset: {e}")
        ...
        if self.config.use_mlflow:
            ...
            with mlflow.start_run(run_name=f"{self.model_name}_auto_optimization", nested=(parent is not None)):
                ...
                auto_model.fit(dataset=dataset_obj)
        else:
            auto_model = _instantiate_model(AutoModelClass, **init_kwargs)
            logger.info(f"Starting optimization with {self.config.num_samples} trials...")
            auto_model.fit(dataset=dataset_obj)
